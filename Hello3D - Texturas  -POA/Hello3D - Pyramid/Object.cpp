...
void Object::initialize(string filePath, string texNames[], Shader* shader, glm::vec3 position, glm::vec3 scale, float angle, glm::vec3 axis)
{
			...
			if (word == "v" || inputFile.eof())
			{
				if (inicioGrupo)
				{
					if (vbuffer.size())
					{
						...
						Mesh grupo;
						GLuint texID = loadTexture(texNames[i]);
						i++;
						int nVerts;
